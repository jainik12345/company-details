...
// app/insert/page.jsx  (SERVER COMPONENT)
import CompanyForm from "@/components/CompanyManager/CompanyForm";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";

// Next.js 16: searchParams is a Promise
export default async function InsertPage({ searchParams }) {
  const params = await searchParams;
  const companyId = params?.edit || null;
  const isEditMode = !!companyId;
  ...
  return (
    <main className="bg-gray-50 py-10">
      <div className="max-w-7xl mx-auto px-4">
        <Link
          href="/"
          className="mb-6 inline-flex items-center cursor-pointer gap-2 text-sm text-gray-600 hover:text-gray-900"
        >
          <ArrowLeft className="w-4 h-4" />
          Back
        </Link>
